...
def load_last_mb_xauusd(file_path=None, mb=16, delimiter=';', col_names=None):
...
def add_indicators(df, include_higher_tf=True):
    ...
    if include_higher_tf:
        ...
        for prefix, freq in (("5m", "5min"), ("1h", "1h")):
...
def train_bot(symbol="XAUUSD"):
    ...
    # itself) — also what gets "5m_"/"1h_"-prefixed for the merged higher-tf candle
    # indicators, since add_indicators() runs this same pipeline on the
    # resampled 5m/1h data and prefixes its output with "5m_"/"1h_" respectively.
...
def test_bot(symbol="XAUUSD"):
    ...
    # bars ≈ 8.3 days (the 5m branch gets a much bigger margin still,
    # ~2400 bars, from this same window). Too small a window here would
    # make the 1h (or 5m) branch come back empty and silently wipe out
    ...
    while True:
        ...
        if current_m5 != last_m5:
            ...
            # elif open_pos != 0 and current_time.date() != last_trading_date:
                ...
                # skipping the 23:54-23:59 window above entirely and
