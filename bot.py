...
def ReversalBlockMitigation(df, lookback=72, threshold=3):
    """
    A rejection-block (bullish_rb/bearish_rb) candle that mitigates — comes
    within `threshold` (raw price; ~30 pips at 0.1/pip, matching
    MSS_Retest's default) — of the most recent swing low/high, within
    `lookback` bars of that swing forming. Mirrors MSS_Retest()'s
    level-tracking pattern, but keys off the raw swing point directly
    rather than requiring an MSS break first.
    """
    high = df["High"].to_numpy()
    low = df["Low"].to_numpy()
    ...
    bullish_rb = df["bullish_rb"].to_numpy()
    bearish_rb = df["bearish_rb"].to_numpy()

    bullish_mitigation = np.zeros(len(df), dtype=np.int8)
    bearish_mitigation = np.zeros(len(df), dtype=np.int8)
    ...
    for i in range(len(df)):
        ...
        # Bullish: a rejection-block candle mitigating the last swing low
        if (
            bullish_rb[i]
            and last_swing_low is not None
            and i - last_swing_low_index <= lookback
            and abs(low[i] - last_swing_low) <= threshold
        ):
            bullish_mitigation[i] = 1
            last_swing_low = None

        # Bearish: a rejection-block candle mitigating the last swing high
        if (
            bearish_rb[i]
            and last_swing_high is not None
            and i - last_swing_high_index <= lookback
            and abs(high[i] - last_swing_high) <= threshold
        ):
            bearish_mitigation[i] = 1
            last_swing_high = None

    return bullish_mitigation, bearish_mitigation
...
def add_indicators(df, include_m15=True):
    ...
    df["bullish_rb_mitigation"], df["bearish_rb_mitigation"] = ReversalBlockMitigation(df)
    ...
    df = df[["Open", "High", "Low", "Close",
            ...
            "bullish_rb_mitigation", "bearish_rb_mitigation",
...
def get_current_recovery_factor(symbol, magic=123456, window=50, target_rf=2.0):
    """
    Recovery factor (net pnl / max drawdown) over the trailing `window`
    closed trades for this symbol/magic, read from MT5's own history —
    used to throttle live SL sizing the same way train_bot's rolling
    reward does. Falls back to `target_rf` (unthrottled) until there are
    at least 2 closed trades or the window has no drawdown to divide by.
    """
    deals = mt5.history_deals_get(datetime(2000, 1, 1), datetime.now() + timedelta(days=1))
    ...
    window_pnls = closed_pnls[-window:]
    dd = max_drawdown(window_pnls)
    total = sum(window_pnls)

    return (total / dd) if dd > 0 else target_rf
...
def train_bot(symbol="XAUUSD"):
    ...
    BASE_INDICATOR_FEATURES = [
        ...
        "EMA7", "EMA21", "EMA721_DIFF", "EMA7_Slope", "EMA21_Slope", "EMA50", "EMA200", "EMA2150_DIFF", "EMA50_Slope", "EMA50200_DIFF", "EMA200_Slope",
        "indecision", "bullish_ob", "bearish_ob", "bullish_fvg", "bearish_fvg", "bullish_ifvg", "bearish_ifvg", "eqh", "eql", "bearish_mb", "bullish_mb", "bullish_rb", "bearish_rb", "bullish_bb", "bearish_bb",
        "bullish_ob_mitigation", "bearish_ob_mitigation", "bullish_fvg_retracement", "bearish_fvg_retracement", "eqh_retest", "eql_retest",
        "swing_high", "swing_low", "bullish_mss", "bearish_mss", "bullish_mss_retest", "bearish_mss_retest", "bullish_mss_retracement", "bearish_mss_retracement",
        "bullish_rb_mitigation", "bearish_rb_mitigation",
    ...
    # Rolling window of the last 50 closed trades, used to reward each
    # trade by the recovery factor of that trailing window (persists
    # across weeks — NOT reset in the weekly stats block).
    rolling_returns = deque(maxlen=50)

    # STANDARD_SL_PIPS = 100
    rr_list = []
    SPREAD_AND_COMMISSION = 0.6

    # Fixed TP / recovery-factor-throttled SL: TP is always TP_PIPS (live
    # position sizing is set so hitting it nets +1% of balance, see
    # open_long/open_short). SL starts at BASE_SL_PIPS and is scaled down
    # proportionally whenever the rolling recovery factor (below) drops
    # below TARGET_RF, so a losing stretch tightens risk automatically
    # instead of holding it fixed.
    TP_PIPS = 20
    BASE_SL_PIPS = 40
    TARGET_RF = 1.0
    MIN_SL_SCALE = 1 / 3   # SL never shrinks below this fraction of BASE_SL_PIPS (100 pips)
    current_rf = TARGET_RF  # unthrottled (full SL) until the rolling window has a meaningful ratio
    ...
    try:
        while True:
            ...
            for i in range(SEQ_LEN, len(df)):
                ...
                # Trigger only once when the stack changes
                ...
                # if action == 1 and not in_position and df["EMA7_Slope"].iloc[i] > 0 and df["EMA21_Slope"].iloc[i] > 0 and df["EMA50_Slope"].iloc[i] > 0 and df["EMA200_Slope"].iloc[i] > 0 and df["k"].iloc[i] < 80 and df["k"].iloc[i] > df["k_smooth"].iloc[i] and df["adx"].iloc[i] > 20:
                ...
                if action == 1:
                    ...
                    # print(f"Entered long, tp: {tp_price:.2f}, entry: {entry_price:.2f}, sl: {sl_price:.2f}, sl pips: {sl_pips:.2f}")
                ...
                # elif action == 2 and not in_position and df["EMA7_Slope"].iloc[i] < 0 and df["EMA21_Slope"].iloc[i] < 0 and df["EMA50_Slope"].iloc[i] < 0 and df["EMA200_Slope"].iloc[i] < 0 and df["k"].iloc[i] > 20 and df["k"].iloc[i] < df["k_smooth"].iloc[i] and df["adx"].iloc[i] > 20:
                ...
                # elif action == 2 and not in_position and df["k"].iloc[i] < df["k_smooth"].iloc[i]:
                    ...
                    # print(f"Entered short, tp: {tp_price:.2f}, entry: {entry_price:.2f}, sl: {sl_price:.2f}, sl pips: {sl_pips:.2f}")
                ...
                if in_position:
                    ...
                    if trade_closed:
                        ...
                        # Rolling recovery factor over the trailing 50 trades:
                        # RF = total pnl / max drawdown of that window. Feeds
                        # the SL throttle (current_rf) only — the PPO reward
                        # below is raw per-trade pnl, not this smoothed RF,
                        # since RF as the reward made the agent trade too
                        # conservatively (~40 trades/week).
                        rolling_returns.append(pnl)
                        window = list(rolling_returns)

                        if len(window) > 1:
                            dd = max_drawdown(window)
                            total = sum(window)
                            rf = total / dd if dd > 0 else total
                            if dd > 0:
                                # feeds the SL throttle for the *next* trade — only
                                # update it from a meaningful ratio, not a raw total
                                current_rf = rf
...
def test_bot(symbol="XAUUSD"):
    ...
    # Fixed TP / recovery-factor-throttled SL — see get_current_recovery_factor().
    # Lot size is set so hitting TP_PIPS always nets +1% of balance; SL starts
    # at BASE_SL_PIPS and shrinks proportionally while the rolling recovery
    # factor over the last 50 closed trades is below TARGET_RF.
    TP_PIPS = 20
    BASE_SL_PIPS = 40
    TARGET_RF = 1.0
    MIN_SL_SCALE = 1 / 3   # SL never shrinks below this fraction of BASE_SL_PIPS (100 pips)
    ...
    while True:
        ...
        # print(f"sleeping {seconds_until_next_minute:.0f} seconds, current time: {datetime.now()}")
        if seconds_until_next_minute <= 0:
            seconds_until_next_minute += 30

        time.sleep(seconds_until_next_minute)
        # print(f"slept {seconds_until_next_minute:.0f} seconds, current time: {datetime.now()}")
        ...
        if current_m5 != last_m5:
            ...
            current_rf = get_current_recovery_factor(symbol, window=50, target_rf=TARGET_RF)
            # sl_scale = min(1.0, max(MIN_SL_SCALE, current_rf / TARGET_RF))
            # base_sl = BASE_SL_PIPS * sl_scale
            ...
            if open_pos == 0:
                ...
                # elif action == 2 and df["EMA7"].iloc[-1] < df["EMA21"].iloc[-1] and df["k"].iloc[-1] > 20:
                    ...
                    # regardless of how wide the (throttled) SL currently is.
                    risk_per_position = min(
                        max((balance * 0.01) / (TP_PIPS * 10), 0.01),
                        100.0
                    )
                    risk_per_position = round(risk_per_position, 2)
