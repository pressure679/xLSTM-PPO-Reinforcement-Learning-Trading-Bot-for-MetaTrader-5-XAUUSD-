...
def train_bot(symbol="XAUUSD"):
    ...
    rint(f"Loaded checkpoint... ({time.strftime('%H:%M')})")
                ...
                if in_position:
                    ...
                    if trade_closed:
                        ...
                        if len(rolling_returns) >= 50:
                            ...
                            avg_sl_now = sum(sl_list) / len(sl_list) if sl_list else 0
                            weekly_pnl_now = np.sum(trade_returns)
                            rf_now = 0.0
                            if max_dd > 0 and avg_sl_now > 0:
                                R_pnl_now = weekly_pnl_now / (avg_sl_now * 2)
                                rf_now = R_pnl_now / (max_dd / 50 / 10)
                                reward = rf_now
                            else:
                                reward = pnl
                            agent.store_transition(
