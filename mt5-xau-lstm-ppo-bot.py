...
def load_last_mb_xauusd(file_path="C:\\Users\\Vittus Mikiassen\\Desktop\\XAU_5m_data.csv", mb=8, delimiter=';', col_names=None):
...
def train_bot(symbol="XAUUSD"):
    ...
    for i in range(SEQ_LEN, len(df)):
        ...
        if action == 1 and not in_position:
            ...
            # prob_multiplier = prob / (1/3)
            # print(f"prob: {prob:.2f}, prob multiplier: {prob_multiplier}")
            # print(f"prob: {prob:.2f}")
            # 0.1 = + 1.05 prob multiplier
            # 50% confidence → ×1.00
            # 70% confidence → ×1.02
            # 90% confidence → ×1.04
            # 99% confidence → ×1.049
