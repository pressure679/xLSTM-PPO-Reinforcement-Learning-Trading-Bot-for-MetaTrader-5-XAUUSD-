...
def train_bot(symbol="XAUUSD"):
    ...
    for i in range(SEQ_LEN, len(df)):
        ...
        base_sl = max(10, current["volume_ma"] / 10)
        ...
        if save_counter % save_count == 0:
            ...
            if len(trade_returns) > 5:
                ...
                print(f"Max DD:          {max_dd*avg_rr/10:.2f}R")
                print(f"PF:              {profit_factor:.2f}")
                print(f"RF:              {round(R_pnl/(max_dd/(avg_sl/10)), 0):.0f}")
                # print(f"RF:              {round(rf, 0):.0f}")
                print(f"Weekly R PnL:    {R_pnl/10:.2f}R")
