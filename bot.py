...
def add_indicators(df):
    ...
    df = df[["Open", "High", "Low", "Close",
            ...
            "bullish_bos", "bearish_bos", "bullish_trend", "bearish_trend",
...
def SelectAction(df, i):
    ...
    # MSS
    buy += int(df["bullish_mss"].iloc[i])
    sell += int(df["bearish_mss"].iloc[i])
...
def train_bot(symbol="XAUUSD"):
    ...
    for i in range(SEQ_LEN, len(df)):
        ...
        if i >= 1440 and i % 1440 == 0:
            for j in range(i - 1440, i):
                if j >= SEQ_LEN and j % (12 * 3) == 0:
                    state_seq = np.array(
                        state_buffer[j - SEQ_LEN:j],
                        dtype=np.float32
                    )
        ...
        high = current["High"]
        low = current["Low"]
        # base_sl = min(50, df["range_ma"].iloc[i])
        base_sl = 100
                ...
                if not trade_closed:
                    if low <= sl_price:
                        if sl_price != entry_price:
                            # sac = SPREAD_AND_COMMISSION if not trade1_open else SPREAD_AND_COMMISSION * 2
                            # pnl -= sl_pips * 2 + sac if trade1_open
