...
def load_last_mb_xauusd(file_path=None, mb=4, delimiter=';', col_names=None):
...
def add_indicators(df, include_higher_tf=True):
    ...
    if include_higher_tf:
        ...
        for prefix, freq in (("15m", "15min"), ("5m", "5min")):
...
def train_bot(symbol="XAUUSD"):
    ...
    # itself) — also what gets "15m_"/"5m_"-prefixed for the merged higher-tf candle
    # indicators, since add_indicators() runs this same pipeline on the
    # resampled 15m/5m data and prefixes its output with "15m_"/"5m_" respectively.
...
def test_bot(symbol="XAUUSD"):
    ...
    # 15m still leaves a comfortable margin over the largest indicator
    # lookback (72 bars) — 3000 1m bars ≈ 200 15m bars ≈ 2.08 days (the
    # 5m branch gets an even bigger margin, ~600 bars, from this same
    # window). Too small a window here would make the 15m (or 5m)
    # branch come back empty and silently wipe out every row via
    # add_indicators()'s dropna().
    ...
    while True:
        ...
        if current_m5 != last_m5:
            ...
            if open_pos != 0:
                trail_stop_loss(symbol, TRAIL_PIPS, TRAIL_STEP_PIPS)
            ...
            elif open_pos != 0 and current_time.date() != last_trading_date:
                # Safety net: XAUUSD/broker feeds commonly go quiet for a
                # few minutes around the daily rollover, so the candle
                # right before midnight can be e.g. 23:50 and the next
                # one to actually arrive already 00:02+ the next day —
                # skipping the 23:55-23:59 window above entirely and
                # leaving a position open overnight. If the calendar date
                # has moved on and we're still holding, close immediately
                # instead of waiting for that night's window.
                close_trades()
                action = 0
